...
input_video = 'video/street.mov'
vid = cv2.VideoCapture(input_video)
video_frame_cnt = int(vid.get(7))
video_width = int(vid.get(3))
video_height = int(vid.get(4))
video_fps = int(vid.get(5))
print("帧数：", video_frame_cnt)
...
##imgs_path = ['image/001.jpg', 'image/002.jpg', 'image/bagong.jpg', 'image/messi.jpg', 'image/bagong1.jpg', 'image/bagong2.jpg', 'image/cat.jpg', 'image/person_cat.jpg', 'image/person_dog.jpg', 'image/person.jpg']
for frame in range(video_frame_cnt):
	...
	# Preprocess the image - normalize to [-1,1](抗锯齿)
	...
	for i in range(0, coreml_scores.shape[0]):
		...
		label = classes_labels[i] + ', {:.2f}%'.format(confidences[i] * 100) + ', {:.2f}ms'.format((end_time - start_time)*1000)
